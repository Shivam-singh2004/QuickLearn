...
function Home() {
    ...
    const fetchCourses = async () => {
      try {
        const response = await axios.get(
          `https://quick-learn-213y.vercel.app/api/v1/course/courses`,
          {
  ...
  const handleLogout = async () => {
    try {
      const response = await axios.get(
        `https://quick-learn-213y.vercel.app/api/v1/user/logout`,
        {
          withCredentials: true,
        }
      );
    ...
    <div className="bg-gradient-to-r from-black to-blue-950 ">
      <div className="h-[1250px] md:h-[1050px] text-white container mx-auto">
          ...
          <div className="space-x-4">
            {isLoggedIn ? (
              ...
              <>
                <Link
                  to={"/login"}
                  className="bg-transparent text-white text-xs md:text-lg md:py-2 md:px-4 p-2 border border-white rounded transition duration-300 ease-in-out hover:bg-white hover:text-black"
                >
                  Login
                </Link>
                <Link
        ...
        <section className="text-center py-20">
          <h1 className="text-4xl font-semibold text-orange-500">QuickLearn</h1>
